...
\- `Cyber\_Attack\_Dashboard.pbix` → Open this in Power BI Desktop to see the dashboard  

\- `cyber\_attack\_analysis.ipynb` → Full Python code (cleaning + ML model)  

\- `powerbi\_main.xlsx` → Main data + ML predictions  

\- `vendor\_risk\_summary.xlsx` → Vendor summaries  

\- `monthly\_trend.xlsx` → Monthly trends  
